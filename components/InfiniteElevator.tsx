...
export default function InfiniteElevator(){
  ...
  const executeRoom=(tier:number,type?:string)=>{
    if(tier===1){const t=type||pick(['NOTHING','LUCKY','HEALTH','MONEY_FOUND','STAIRS_SHORT','DOORS','SHOP_SMALL','TREASURE','FORTUNE','BOXES','BARTER','CROSSROADS']);
      ...
      else if(t==='LUCKY'){const g=ri(1,3);patch(current=>({luck:current.luck+g}));show({tier,title:'ラッキー部屋',desc:'温かい光。運気UP！',result:`運気 +${g}`,resultType:'success'});}
      else if(t==='HEALTH'){patch(current=>({turnsLeft:current.turnsLeft+1}));show({tier,title:'健康の湯',desc:'温泉で一休み。',result:'残り回数 +1',resultType:'success'});}
      else if(t==='MONEY_FOUND'){const g=ri(100,500);patch(current=>({money:current.money+g}));show({tier,title:'落ちている財布',desc:'足元に財布が落ちていた！',result:`+${g}円 拾った！`,resultType:'gold'});}
      else if(t==='STAIRS_SHORT'){const g=ri(5,15);patch(current=>({floor:current.floor+g}));show({tier,title:'短い階段',desc:'一気に駆け上がった！',result:`+${g}階上へ`,resultType:'success'});}
      ...
      else if(t==='TREASURE'){const g=ri(300,900);patch(current=>({money:current.money+g}));show({tier,title:'小さな宝箱',desc:'古びた宝箱を見つけた。',result:`+${g}円`,resultType:'gold'});}
      ...
      else if(t==='SUPER_LUCKY'){const g=ri(3,5);patch(current=>({luck:current.luck+g}));show({tier,title:'超ラッキー部屋',desc:'強い運気のオーラ！',result:`運気 +${g}`,resultType:'success'});}
      else if(t==='HEALTH_2'){const g=ri(2,3);patch(current=>({turnsLeft:current.turnsLeft+g}));show({tier,title:'無病の湯',desc:'元気が出た！',result:`残り回数 +${g}`,resultType:'success'});}
      else if(t==='RUBY_MINING')setupMining(tier,'ruby');
      else if(t==='STAIRS_MED'){const g=ri(15,30);patch(current=>({floor:current.floor+g}));show({tier,title:'長い階段',desc:'螺旋階段を登った！',result:`+${g}階上へ`,resultType:'success'});}
      ...
      else if(t==='SUPER_LUCKY_3'){const g=ri(6,8);patch(current=>({luck:current.luck+g}));show({tier,title:'極ラッキー部屋',desc:'祝福の光！',result:`運気 +${g}`,resultType:'gold'});}
      else if(t==='HEALTH_3'){const g=ri(4,5);patch(current=>({turnsLeft:current.turnsLeft+g}));show({tier,title:'不老不死の湯',desc:'圧倒的な活力！',result:`残り回数 +${g}`,resultType:'gold'});}
      else if(t==='EMERALD_MINING')setupMining(tier,'emerald');
      else if(t==='STAIRS_LONG'){const g=ri(50,200);patch(current=>({floor:current.floor+g}));show({tier,title:'果てしなく続く階段',desc:'次元を超える階段！',result:`+${g}階上へ`,resultType:'gold'});}
  ...
  const interactive=useMemo(()=>{
    ...
    if(kind==='vending')return <SimpleGrid columns={2} spacing={2}><Action title="運気ドリンク" sub="200円 (50%で+1)" onClick={()=>{if(s.money<200){playSfx('fail',soundOn);return;}playSfx('buy',soundOn);patch(current=>({money:current.money-200,luck:current.luck+(Math.random()<.5?1:0)}))}}/><Action title="回数ドリンク" sub="400円 (50%で+1)" onClick={()=>{if(s.money<400){playSfx('fail',soundOn);return;}playSfx('buy',soundOn);patch(current=>({money:current.money-400,turnsLeft:current.turnsLeft+(Math.random()<.5?1:0)}))}}/></SimpleGrid>;
    ...
    if(kind==='casino')return <Stack spacing={2}>
      ...
      <Button colorScheme="purple" size="sm" isLoading={slotSpinning} loadingText="リール回転中…" onClick={()=>{
        ...
        const sy=['🔴','🟢','💎','🎡'];
        // 3つ揃いの実確率は従来どおり（独立4絵柄 = 1/16）。
        // ハズレだった場合だけ一部を「2リール同柄」に見せ、リーチ体験を少し増やす。
        const natural=[pick(sy),pick(sy),pick(sy)];
        const naturalWin=natural[0]===natural[1]&&natural[1]===natural[2];
        let final=[...natural];
        if(!naturalWin && final[0]!==final[1] && Math.random()<.32){
