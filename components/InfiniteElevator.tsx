...
export default function InfiniteElevator(){
  ...
  const [finalEventInteracted,setFinalEventInteracted]=useState(false);
  ...
  const start=()=>{playSfx('start',soundOn);setForgeUsed(false);setPendingGameEnd(false);setFinalEventInteracted(false);setS({...baseState,highScore:s.highScore});setMenu(false);setGameover(false);setDoors(true);setRoom({tier:1,title:'エレベーターホール',desc:'エレベーターに乗りました。ボタンを押して上の階を目指しましょう！'});};
  ...
  useEffect(()=>{
    if(!pendingGameEnd || moving || gameover || room.kind) return;
    const timer=window.setTimeout(()=>{setPendingGameEnd(false);setFinalEventInteracted(false);end();},900);
    return ()=>window.clearTimeout(timer);
  },[pendingGameEnd,moving,gameover,room.kind,room.result]);
  ...
  const interactive=useMemo(()=>{
    ...
    if(kind==='mystery')return <Center w="100%"><Box w="100%" maxW="280px"><Action title="商品を買う（1000円）" sub="謎の商品を1つ落札する" onClick={()=>{if(s.money<1000){playSfx('fail',soundOn);show({...room,result:'所持金が足りない…',resultType:'danger'});return;}playSfx('buy',soundOn);setS(x=>({...x,money:x.money-1000}));const r=Math.random();if(r<.4){const item=Math.random()<.5?makeItem('mirror',4):makeItem('ring',8);addItem(item);playSfx('item',soundOn);show({...room,kind:undefined,result:`落札商品：${item.name} を獲得！`,resultType:'gold'});}else if(r<.8){setS(x=>({...x,money:x.money+1500}));playSfx('coin',soundOn);show({...room,kind:undefined,result:'落札商品：賞金袋 1,500円を獲得！',resultType:'gold'});}else{playSfx('fail',soundOn);show({...room,kind:undefined,result:'落札商品：空っぽの袋だった…',resultType:'neutral'});}}}/></Box></Center>;
  ...
  const disabled=pendingGameEnd ? (moving||gameover||slotSpinning||bj.playing) : (moving||gameover||s.turnsLeft<=0||slotSpinning||bj.playing||s.inHell);
  ...
  return <Center h="100dvh" minH={0} p={{base:0,md:4}} overflow="hidden">
    <Box onClickCapture={handleButtonSound} w="100%" maxW="432px" h={{base:'100dvh',md:'min(860px, calc(100dvh - 32px))'}} maxH={{base:'100dvh',md:'calc(100dvh - 32px)'}} bg="#0f141d" borderRadius={{base:0,md:'3xl'}} overflow="hidden" position="relative" borderWidth={{base:0,md:'4px'}} borderColor="whiteAlpha.200" boxShadow="2xl">
      ...
      <Flex h="100%" direction="column">
        ...
        <Box flexShrink={0} bg="gray.950" p={3} borderTop="1px solid" borderColor="gray.800"><Button w="100%" h="56px" bgGradient={pendingGameEnd?"linear(to-r, red.500, orange.500)":"linear(to-r, cyan.400, blue.500)"} color="white" fontSize="md" fontWeight="900" textShadow="0 1px 3px rgba(0,0,0,.95)" border="1px solid" borderColor={pendingGameEnd?"red.200":"cyan.200"} boxShadow={pendingGameEnd?"0 4px 14px rgba(255,80,80,.28)":"0 4px 14px rgba(0,180,255,.28)"} _hover={{bgGradient:pendingGameEnd?'linear(to-r, red.400, orange.400)':'linear(to-r, cyan.300, blue.400)',color:'white'}} _active={{bgGradient:pendingGameEnd?'linear(to-r, red.600, orange.600)':'linear(to-r, cyan.600, blue.700)',color:'white',transform:'translateY(1px)'}} _disabled={{opacity:.55,color:'whiteAlpha.800'}} leftIcon={pendingGameEnd?undefined:<FaArrowUp/>} isDisabled={disabled} onClick={()=>{if(pendingGameEnd){setPendingGameEnd(false);setFinalEventInteracted(false);end();}else press();}}>{pendingGameEnd?'ゲームを終了する':'ボタンを押す'}</Button></Box>
      </Flex>
